...
public class Terrain extends Drawable
{
	...
	public void explodeRhombus(float leftX, float leftY, float rightX, float rightY, float height)
	{
		...
		int max_index = Math.min(NUM_POINTS - 1, (int) (rightX / segmentWidth) + 1);
		...
		{
			num_dirts = _generateDirtpointsRhombus(min_x, max_x, leftX, leftY, rightX, rightY, height);
			totalDirtVolume = _collapseToRhombusBottom(min_index, max_index, leftX, leftY, rightX, rightY, height);
...
//			 }
	...
	/*
	 * Sets terrain heights to the bottom of a circle at (x, y) with radius r Also returns the total amount of land removed between the top of the circle and
	 * the current land height
	 */
	private float _collapseToRhombusBottom(int min_index, int max_index, float leftX, float leftY, float rightX, float rightY, float height)
	{
		float dirtVolume = 0;
		for (int i = min_index; i <= max_index; i++)
		{
			int min_x = (int) (min_index * segmentWidth);
			float xFromI = i * segmentWidth;
			float offset = leftY + (i * segmentWidth - min_index * segmentWidth) * ((rightY - leftY) / (rightX - leftX));
			float tCircleY = offset + height;
			float bCircleY = offset;
	...
	/*
	 * public void dropDirt(float x, float y) { PhysicsController physicsController = PhysicsController.getInstance(this); Dirt dirtPoint = new Dirt();
	 * dirtPoint.p[0] = x; dirtPoint.p[1] = y; dirtPoint.v[1] = -1; dirtPoint.width = DIRT_SIZE; dirtPoint.height = DIRT_SIZE; dirtPoint.volume = 1;
	 * physicsController.addCollidable(dirtPoint); registerDrawable(dirtPoint); }
	 */
}