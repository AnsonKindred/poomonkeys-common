...
public class Terrain extends Drawable
{
	...
	public void explodeRectangle(float x, float y, float r)
	{
		...
		synchronized(this)
		{
			dirt = _generateDirtpointsRectangle(min_x, max_x, x, y, r);
	...
	private ArrayList<Dirt> _generateDirtpointsRectangle(float min_x, float max_x, float x, float y, float r)
	{
		...
		{
			_generateDirtColumnRectangle(col_x, x, y, r, gap, dirt);
		}

		return dirt;
	}
	
	private float _generateDirtColumnRectangle(float col_x, float x, float y, float r, float gap, ArrayList<Dirt> dirt)
	{
