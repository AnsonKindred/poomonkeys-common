...
public class Tank extends Drawable {
	...
	public void intersectTerrain(Terrain t, float[] intersect) {
		...
		if (force > 5000) {
			...
			{
				float angle = (float) Math.atan((rightHeight - leftHeight)
						/ t.segmentWidth);
				System.out.println(angle);
				this.v[0] = (float) (groundSpeed * Math
						.cos(2 * Math.PI - angle));
				this.v[1] = (float) (groundSpeed * -Math.sin(2 * Math.PI
						- angle));
			...
			} else if (lDiff > 0)// && lDiff >= rDiff) {
			{
				...
				this.v[0] = (float) (groundSpeed * Math
						.cos(Math.PI - angle));
				this.v[1] = (float) (groundSpeed * -Math.sin(Math.PI
						- angle));
	...
	public void underTerrain(Terrain t) {
		...
		float rPercentX = ((this.p[0] + width / 2) / t.segmentWidth)
				- (int) ((this.p[0] + width / 2) / t.segmentWidth);
		float rLandY = t.points[(int) (rightX / t.segmentWidth)]
				+ (t.points[(int) (rightX / t.segmentWidth) + 1] - t.points[(int) (rightX / t.segmentWidth)])
				* rPercentX;
		float lPercentX = (((this.p[0] - width / 2) / t.segmentWidth) - (int) ((this.p[0] - width / 2) / t.segmentWidth));
		float lLandY = t.points[(int) (leftX / t.segmentWidth)]
				+ (t.points[(int) (leftX / t.segmentWidth) + 1] - t.points[(int) (leftX / t.segmentWidth)])
				* lPercentX;
